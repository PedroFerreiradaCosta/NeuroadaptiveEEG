%% Nc_continuum: Nc for faces from the mum-stranger continuum 

% Adapted from the Nc_gazeXemotion.m script

% last edited: AG, 11/04/2021
...
% number of monitors
NumScreen = 1; %change to 2 in 202, change to 1 if only 1 screen attached
...
Ntrials = 3; %number of trials within block
NBlcks = 3; %number of blocks

% Numbers of good ERPs
N_goodQualERP_aim = 3;    
perc_good_ts_wished = 60; % to define
...
TimeInfo.Protocol = 'OnlineEEG_wholescalp'; %'OnlineEEG_checkers'  'OnlineEEG_wholescalp' 'OnlineEEG_wholescalp_wLS'
...
%% define session parameters and paths
...
%% if python ready, just run from here
...
%% needed if running on offline data 
% eeg = EEGdata_raw{1,1}.eeg  
% mark = EEGdata_raw{1,1}.mark         
% t_mark = EEGdata_raw{1,1}.t_mark         
% eeg_t = EEGdata_raw{1,1}.eeg_t  
% Ntrials = 12
%FigChQual = figure; % Channel feedback
%set(gcf,'position',[1112 385 781 330],'MenuBar','none')
%plot(0,0,'k','Marker','.','MarkerSize',20)
...
%      perc_good = 100 - perc_bad
    ...
    % Channel feedback; included in ERP or not
     [FigChQual,Incl] = SD_Channel_Feedback(ChExcl_8, TimeInfo, FigChQual, Ntrials, Blck, N_goodQualERP_aim, perc_good);
...
%% check data qual and decide whether to repeat
    ...
    if contains(str1,'c')
    %if perc_good > perc_good_ts_wished == 1
    ...
    elseif contains(str1,'r')
                    ...
                    dlmwrite('/Users/braintools/Desktop/BONDS_SD/LastBlockOnline.txt',Blck);
 %                 return
%% switch to offline stimulus presentation

                    addpath('/Users/braintools/Desktop/BONDS_SD/1dBO/images');
                    pres.LoadStim('/Users/braintools/Desktop/BONDS_SD/1dBO/images')
                    ...
                    % Channel feedback; included in ERP or not
                         [FigChQual,Incl] = SD_Channel_Feedback(ChExcl, TimeInfo, FigChQual, Ntrials, Blck, N_goodQualERP_aim, perc_good);
    ...
    %         prompt = 'Low EEG quality - Continue session? y/n';
    %         str2 = input(prompt,'s');
    % 
    %             if  contains(str2,'n')
    %                 OutputVal = 999;  %save 999 in Output file for py
    ...
    % 
    %                 disp('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%')
    %                 disp('End of the session (low qual EEG)')
    %                 disp('%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%')
    %                 return
    %             else
    %                 rep_text = sprintf('Repeating block %2d',Blck);
